...
![understanding Devcontainers](./understanding-devcontainers.png)
...
![Docker Image](https://docs.docker.com/engine/images/architecture.svg)

### Terminology

##### *Docker Image*

A Docker image consists of instructions on creating a Docker container. The images can be customized to suit your needs by specifing a `Dockerfile`. Each line within the simple syntax is an instruction and considered a layer within the image. Note, you should limit the number of layers an image has to speed up the build time. Docker's [documentation](https://docs.docker.com/develop/develop-images/dockerfile_best-practices/) and [this](https://www.youtube.com/watch?v=Z1Al4I4Os_A) PyData talk, give great insight into speeding up your build time and the best practices on using the `Dockerfile`. 
...
- [Docker Beginnner Video](https://www.youtube.com/watch?v=iqqDU2crIEQ): How to get started with Docker
...
# install the requirments file by using the RUN command
...
![Docker Containers](what-is-container.png)
...
This seperates your services into isolate environments but still allows them to run together. The application's configuration can be specified within the `docker-compose.yml` file. Then, with a single command, you create and start all the services from your configuration.

![Docker Compose Example](./Docker-Compose-Example.png)
...
![Container Name Junction ](./container-name-junction-model.png)
...
Your source code is outside of the container, (i.e. within your local machine), so you no longer have to rebuild the contaier for small code changes. You only need to rebuilt it when you make changes to Docker related files. If you use Anaconda as a base image, then you won't need to do that much.

![Local Development](./local-development-environments.jpg)
