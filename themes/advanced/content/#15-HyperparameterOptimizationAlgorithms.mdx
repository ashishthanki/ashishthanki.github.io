...
cover: images/HPO/HPO_Libaries.png
...
There are many Hyperparameter Optimization (HPO) libraries to aid in finding the best hyperparameters for ML models but all of them incorporate different search algorithms, consequently, outputing different parameters and model scores. The library to use ultimately depends on the machine learning problem that you are trying to solve. 

Nonetheless, the two most popular HPO search algorithms: Bayesian Optimization and Sequential model-based optimization (SMBO). These methods are far superior when compared to exhaustive methods such as grid search and randomized grid search. 

The issues with the exhaustive methods is that we do not use past evaluation results to select next hyperparameters to evaluate. It evaluates all possible combinations (or number of defined iterations, `n_iters`, for randomized) within the parameter grid and reports back the best combination. 
